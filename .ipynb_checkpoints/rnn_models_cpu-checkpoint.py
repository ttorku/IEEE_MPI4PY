...
def evaluate_model(model, x_test, y_test, x_data, y_data, x_train, y_train,scaler1):
    ...
    output=out.detach().numpy().reshape((-1,1))
    ...
    out, h = model(inputs_train.float(), h)
    output1=out.detach().numpy().reshape((-1,1))
    ...
    out, h = model(inputs_test.float(), h)
    output2=out.detach().numpy().reshape((-1,1))
...
out_size = 1
n, l =16, 2
def run_model_cpu(data, option, epochs, batch_size, lookback, split):
    print('{} outcomes on CPU............'.format('Tennessee'))
